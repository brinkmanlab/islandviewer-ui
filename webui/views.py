...
WRITE_ERROR = "<h1>Unable to write {}</h1><pre>{}</pre>"
READ_ERROR = "<h1>Unable to parse Genbank</h1>" \
             "<pre>{}</pre>"\
             "<p>If your data was generated by Prokka, please "\
             "ensure you use Prokkas --complaint flag to generate standard compliant output. IslandViewer will not be able to process it otherwise.</p>"
...
def fetchislands(request):
    ...
    try:
        ...
        response.content = READ_ERROR.format(e.message)
        return response
    recs = p.fetchRecords()
...
def downloadCoordinates(request):
    ...
    try:
        ...
        response = HttpResponseServerError(reason='Unable to parse Genbank')
        response.content = WRITE_ERROR.format(format, e.message)
        return response
