...
def fetchGenbankFile(accession):
    ...
    cursor.execute("SELECT genomeproject.gpv_directory FROM genomeproject, replicon WHERE genomeproject.	gpv_id = replicon.gpv_id AND replicon.rep_accum = %s",[accession])
